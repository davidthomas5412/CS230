...
def train(config):
	...
	for i, sample in enumerate(config.dataloader()):
		...
		if toscalar(binlabel_batch) > 0:
			# np.save('data/binary/label/v{}.npy'.format(varcounter), label)
			np.save('data/binary/train/var/{}.npy'.format(varcounter), output)
			varcounter += 1
		else:
			# np.save('data/binary/label/nv{}.npy'.format(nonvarcounter), label)
...
class AugmentedDevData(Dataset):
	...
	def __getitem__(self, idx):
		...
		if np.any(lab > 1) and np.sum(lab > 1) < 200:
			return self.__getitem__(idx + 3000)
		elif np.any(lab > 1):
			binlabel = np.sum(lab > 1) > 200
		else:
			binlabel = False
