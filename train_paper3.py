...
"""
For using weight decay to eliminate extreme behavior.
"""
...
def train(config):
	...
	for epoch in range(config.epochs):
		...
		for i, sample in enumerate(config.dataloader(mode='train')):
			...
			loss = mse(output_batch, label_batch)
			...
			if i % config.steps_per_dev_loss == 0:
				...
				for j, dev_sample in enumerate(config.dataloader(mode='dev')):
					...
					dloss_arr.append(toscalar(mse(output_batch, label_batch)))
...
class SimpleConfig:
	def __init__(self):
		...
		self.checkpoint_path = 'experiments/checkpoint_paper3'
		self.best_checkpoint_path = 'experiments/checkpoint_paper3_best'


		self.model = Net()
		self.optimizer = Adam(self.model.parameters(), lr=.01, weight_decay=0.001) # so that weight of 10 would contribute a lot to loss
